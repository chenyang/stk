(function() {
    ...
    mod.controller("StockAnalysisCtrl", ['$scope', '$modal', '$cookies', '$location', '$http', '$routeParams',
        ...
        function($scope, $modal, $cookies, $location, $http, $routeParams,
            ...
            var setGraphOptions = function() {
                $scope.options = {
                    chart: {
                        ...
                        x: function(d){ return d3.time.format('%Y%m%d')(new Date(d.date)); },
                        y: function(d){ return d3.format(',.2f')(d.value); },
                        interactive: true,
                        showLegend:false,
            ...
            function adaptGraphData(dataArray) {
                ...
                return [{
                    ...
                    key: '收盘价',
                    color: '#7AC5CD' //color - optional: choose your own line color.
                }];
            }
            ...
            $scope.gotoCoverpage = function() {
                $location.path("/buyPub/" + $routeParams.pubId);
            };
            ...
            var init = function() {
                ...
                if (_.isEmpty($cookies.getObject('cookieAllVisitedPubId'))) {
                    ...
                    cookieAllVisitedPubId.push($routeParams.pubId);
                    $cookies.putObject('cookieAllVisitedPubId', cookieAllVisitedPubId);
                } else {
                    var hasCookied = false;
                    for (var i = 0; i < cookieAllVisitedPubId.length; i++) {
                        if (cookieAllVisitedPubId[i] == $routeParams.pubId) {
                            hasCookied = true;
                        }
                    }
                    if (!hasCookied) {
